...
class YuvToRgbConverter(context: Context) {
    ...
    fun yuvToRgb(image: Image , output:Bitmap)
    {
        if(!::yuvBuffer.isInitialized){
            pixelCout = image.cropRect.width()*image.cropRect.height()
            val pixelSizeBits = ImageFormat.getBitsPerPixel(ImageFormat.YUV_420_888)
            yuvBuffer = ByteBuffer.allocate(pixelCout*pixelSizeBits/8)
        }
        yuvBuffer.rewind()
        imageToByteBuffer(image , yuvBuffer.array())

        if(!::inputAllocation.isInitialized){
            val elemType= Type.Builder(rs , Element.TYPE(rs)).setYuvFormat(ImageFormat.NV21).create()
    ...
    private fun imageToByteBuffer(image: Image, outputBuffer: ByteArray) {
        ...
        imagePlanes.forEachIndexed{planeIndex , plane ->
            ...
            when(planeIndex){
                ...
                1 ->{
                    outputStride=2
                    outputOffset= pixelCout+1
                }
                2 ->{
                    outputStride=2
                    outputOffset= pixelCout
                }
                else ->{
